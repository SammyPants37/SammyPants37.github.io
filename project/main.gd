	...
	for row in map: # place the walls and pillar to win
		...
		for i in row:
			if i == "░":
				...
				# hide the lights on either side of the exit staircase because the poke through
				if currCol == 24 and (currRow == 22 or currRow == 24):
					wall.hideLights()
				add_child(wall)
			elif i == "P":
